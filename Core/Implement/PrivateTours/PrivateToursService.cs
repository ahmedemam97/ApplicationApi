...
namespace Application.Implement.PrivateTourss
{
    public class PrivateToursService: IPrivateToursService
    {
        ...
        public async Task<ResponseResult> Create(PrivateTours model)
        {
            if (model == null)
                return new ResponseResult { IsSuccess = false, Message = "Null" };

            if (model.Name == "" || model.Name == null)
                return new ResponseResult { IsSuccess = false, Message = "ادخل اسم الرحله " };
            ...
            await UnitOFWork.Repository<PrivateTours>().AddAsync(model);
        ...
        public async Task<ResponseResult> Delete(int id)
        {
            ...
            var modelExist = await UnitOFWork.Repository<PrivateTours>().
                Where(e => e.Id == id).FirstOrDefaultAsync();
            if (modelExist == null)
                return new ResponseResult { IsSuccess = false, Message = "NotFound" };

            var i = await UnitOFWork.Repository<PrivateTours>().DeleteAsync(modelExist.Id);
            await UnitOFWork.CompleteAsync();
        ...
        {
            var All = await UnitOFWork.Repository<PrivateTours>().ToListAsync();
            if (All == null)
        ...
        public async Task<ResponseResult> Update(PrivateTours model)
        {
            if (model == null)
            ...
            var i = await UnitOFWork.Repository<PrivateTours>().UpdateAsync(MapModel);
            await UnitOFWork.CompleteAsync();
